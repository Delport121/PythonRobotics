...
def update_with_observation(particles, z):
    for iz in range(len(z[0, :])):
        ...
        for ip in range(N_PARTICLE):
            # new landmark
            # All particles are initialised as 0 so if there is a 
            # new landmark detected it will creat a new one based on this condition
...
def compute_weight(particle, z, Q_cov):
    lm_id = int(z[2])
    xf = np.array(particle.lm[lm_id, :]).reshape(2, 1)  # Landmark position
    Pf = np.array(particle.lmP[2 * lm_id:2 * lm_id + 2]) # Landmark covariance
...
def compute_jacobians(particle, xf, Pf, Q_cov):
    ...
    # Jacobian with Respect to the Vehicle State
    Hv = np.array([[-dx / d, -dy / d, 0.0],
                   [dy / d2, -dx / d2, -1.0]])

    # Jacobian with Respect to the Landmark State
...
def update_landmark(particle, z, Q_cov):
    # Extract Landmark ID, Position, and Covariance 
    lm_id = int(z[2])
    xf = np.array(particle.lm[lm_id, :]).reshape(2, 1)    # Landmark position
    Pf = np.array(particle.lmP[2 * lm_id:2 * lm_id + 2, :])  # Landmark covariance

    # zp: Predicted Measurement
    # Hv: Jacobian with Respect to the Vehicle State
    # Hf: Jacobian with Respect to the Landmark State
    # Sf: Innovation Covariance
    zp, Hv, Hf, Sf = compute_jacobians(particle, xf, Pf, Q)

    # The innovation dz is the difference between the actual measurement z and the predicted measurement zp. 
    # The bearing component is normalized using pi_2_pi to ensure it is within the range [−𝜋,𝜋] [−π,π].
    #  dz = z - z_predict   
    dz = z[0:2].reshape(2, 1) - zp
    dz[1, 0] = pi_2_pi(dz[1, 0])

    # Update Landmark Position and Covariance Using EKF
    xf, Pf = update_kf_with_cholesky(xf, Pf, dz, Q_cov, Hf)

    # Store the updated landmark position and covariance in the particle
...
def update_kf_with_cholesky(xf, Pf, v, Q_cov, Hf):
    #1 Compute Intermediate Matrices:
    PHt = Pf @ Hf.T         # Covariance extrapolation
    S = Hf @ PHt + Q_cov
    #2 Symmetrize S and Perform Cholesky Decomposition:
    S = (S + S.T) * 0.5
    s_chol = np.linalg.cholesky(S).T
    s_chol_inv = np.linalg.inv(s_chol)
    #3 Compute Kalman Gain:
    W1 = PHt @ s_chol_inv # K = Pf @ Hf.T @ inv(S)
    W = W1 @ s_chol_inv.T
    #4 Update Landmark Position and Covariance:
    x = xf + W @ v  
    P = Pf - W1 @ W1.T

    return x, P
    #This function seeem to write the kalman filter equation in very wierd way
...
# Functions for state transition and observation
def observation(xTrue, xd, u, rfid):
    ...
    # Returns the true state, the observation data, the dead reckoning state, and the input data
    # z the distance and angles such as with a lidar as well as the landmark index
    # z = [distance, angle, i]'
...
#Converts angle to the range of -pi to pi
def pi_2_pi(angle):
