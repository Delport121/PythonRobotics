...
def plot_observation_line(x, z, ax=None, linewidth=1.0, color="k"):
    for i in range(z.shape[1]):  # Iterate over columns since z is now (3, N)
        ...
        # Compute landmark coordinates in global frame
        x_l = x[0, 0] + range_obs * math.cos(x[2, 0] + angle_obs)
        y_l = x[1, 0] + range_obs * math.sin(x[2, 0] + angle_obs)
        
        # Plot the line from the robot's position to the landmark
        if ax is None:
            plt.plot([x[0, 0], x_l], [x[1, 0], y_l], "-", color=color, linewidth=linewidth, zorder = 2,label="Lidar Observation")
        else:
            ax.plot([x[0, 0], x_l], [x[1, 0], y_l], "-", color=color, linewidth=linewidth, zorder = 2,label="Lidar Observation")
...
def observation(x_true, xd, u, rfid):
    ...
    # Propagate the noisy input through the motion model
    xd = motion_model(xd, ud)
